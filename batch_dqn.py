...
class DQN:
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope("target_dqn"):
            ...
            self.target_estimator = Estimator(observation_length, is_target_dqn=False, var_scope_name="target_dqn", bias_average=bias_average)

        # ESTIMATOR
        with tf.compat.v1.variable_scope("dqn"):

            self.estimator = Estimator(observation_length, is_target_dqn=True, var_scope_name="dqn", bias_average=bias_average)
        ...
        self.summary_writer = tf.compat.v1.summary.FileWriter(summary_dir, graph=tf.get_default_graph())
    ...
    def get_action(self, dataset, model, state, next_action_batch, next_action_unlabeled_data):
        ...
        predictions = self.session.run(
            ...
            feed_dict = {self.estimator.classifier_placeholder: state, 
                         self.estimator.action_placeholder: next_action_batch})
        selected_batch = np.random.choice(np.where(predictions == predictions.max())[0])
        selected_batch = int(next_action_batch[selected_batch])

        train_predictions = model.predict_proba(dataset.train_data[next_action_unlabeled_data, :])[:,0]

        # Uncertainty sampling.
        uncertainty_scores = -np.abs(train_predictions)  # Since decision_function returns raw margin scores, we need to take absolute value and negate.
